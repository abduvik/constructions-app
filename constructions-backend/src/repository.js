...
const createConstructionsRepository = () => {
  return constructionCompaniesNames.map((name, index) => {
    const specialities = [];
    const specialitiesToAdd =
      Math.floor(Math.random() * constructionCompaniesSpecialities.length) + 1;
    const remainingSpecialities = constructionCompaniesSpecialities.slice();

    for (let i = 0; i < specialitiesToAdd; i++) {
      const randomIndex = Math.floor(
        Math.random() * remainingSpecialities.length
      );
...
createSpecialitiesRepository = () => constructionCompaniesSpecialities;
